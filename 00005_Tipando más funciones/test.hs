describe "mismaCantidadDeLetras" $do 
  it "es del tipo correcto" $ do
    ...
    Exception.evaluate (mismaCantidadDeLetras ([1,2,3] :: [Int]) ([1,2,3] :: [Int])) `shouldThrow` anyException
...
describe "primerasNLetrasAlReves" $do 
  it "es del tipo correcto" $ do
    ...
    Exception.evaluate (primerasNLetrasAlReves (1 :: Int) ([1,2,3 :: [Int]])) `shouldThrow` anyException
